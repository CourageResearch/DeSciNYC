...
export default async function HomePage() {
  ...
  const upcomingEvents = db.events.filter(e => parseInt(e.id) >= parseInt(nextEventId));
  const lumaEvents = await Promise.all(upcomingEvents.map(e => getLumaEvent({ event_id: e.luma_id })));
